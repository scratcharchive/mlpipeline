...
if [[ "$QT_VERSION" =~ ^4.*$ ]]
then
  echo "You're trying to build MLPipeline with Qt4 ($QT_VERSION) but MLPipeline only supports Qt5"
...
if [[ $EXIT_CODE -ne 0 ]]; then
	...
	RES=$(which mlpipeline)
	if [[ -z $RES ]]; then
		echo ""
		echo "******************************************"
		echo "It appears that mlpipeline is not"
		echo "found in your path. Be sure to add"
		echo "mlpipeline/bin to your path. See the"
		...
		if [[ -z $RES ]]; then
			...
			echo "found in your path. Be sure to install"
			echo "mountainlab. Otherwise, not much will work."
			echo "******************************************"
		fi
