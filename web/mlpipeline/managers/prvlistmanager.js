...
function PrvListManager(O) {
  ...
  */
  ...
  function checkOnRB(name) {
    ...
    if (!prvrec) return;
    if (prvrec.content) return;
    if (!m_rb_download_manager) {
      console.error('RB Download manager not set in PrvListManager.');
      set_on_rb_val(undefined);
      return;
    }
    m_rb_download_manager.stat(prvrec.prv.original_checksum,function(err,tmp) {
      if (err) {
        console.log ('Error checking on RB: '+err);
        set_on_rb_val(undefined);
        return;
      }
      if (!tmp.found) {
      ...
      set_on_rb_val(true);
      m_rb_download_manager.probeDownload(prvrec.prv.original_checksum,function(err,tmp2) {
        if (err) {
          set_downloading_to_server_val(false,'');
          return;
        }
        if (tmp2.status=='starting') {
          set_downloading_to_server_val(true,'starting...');
        }
        else if (tmp2.status=='running') {
          set_downloading_to_server_val(true,'downloading...');
        }
        else if (tmp2.status=='error') {
          set_downloading_to_server_val(true,'error: '+tmp2.error);
        }
        else {
          set_downloading_to_server_val(false,tmp2.status)
        }
      });
    });
    function set_downloading_to_server_val(val,message) {
      prvrec.downloading_to_server=false;
      prvrec.downloading_to_server_message=message;
    }
  }
  */
